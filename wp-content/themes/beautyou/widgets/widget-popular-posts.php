...
class themerex_popular_posts_widget extends WP_Widget {
	...
	function themerex_popular_posts_widget() {
		...
		/* Create the widget. */
		$this->WP_Widget('themerex-popular-posts-widget', __('AxiomThemes - Most Popular & Commented Posts', 'themerex'), $widget_ops, $control_ops);
	}
	...
	function form($instance) {
		...
		<p>
			...
			<input type="radio" id="<?php echo esc_attr($this->get_field_id('show_counters')); ?>_2" name="<?php echo esc_attr($this->get_field_name('show_counters')); ?>" value="1" <?php echo esc_attr($show_counters==1 ? ' checked="checked"' : ''); ?> />
