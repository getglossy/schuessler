...
class themerex_recent_posts_widget extends WP_Widget {
	...
	function themerex_recent_posts_widget() {
		...
		/* Create the widget. */
		$this->WP_Widget('themerex-recent-posts-widget', __('AxiomThemes - Recent Posts', 'themerex'), $widget_ops, $control_ops);
	}
	...
	function form($instance) {
		...
		<p>
			...
			<?php
				foreach ($categories as $cat_id => $cat_name) {
					echo '<option value="'.esc_attr($cat_id).'"'.($category==$cat_id ? ' selected="selected"' : '').'>'.esc_attr($cat_name).'</option>';
				}
			?>
			</select>
		</p>
		...
		<p>
			...
			<input type="radio" id="<?php echo esc_attr($this->get_field_id('show_counters')); ?>_2" name="<?php echo esc_attr($this->get_field_name('show_counters')); ?>" value="1" <?php ($show_counters==1 ? ' checked="checked"' : ''); ?> />
