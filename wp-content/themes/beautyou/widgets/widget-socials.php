...
class themerex_social_widget extends WP_Widget {
	...
	function themerex_social_widget() {
		...
		/* Create the widget. */
		$this->WP_Widget( 'themerex-social-widget', __('AxiomThemes - Show logo and social links', 'themerex'), $widget_ops, $control_ops );
	}
		...
		<div class="widget_inner">
            <?php
				if ($show_logo) {
					...
					?>
						<div class="logo"><a href="<?php echo esc_url(home_url()); ?>"><?php echo (esc_html($logo_text) ? '<span class="logo_text">'.str_replace(array('[', ']'), array('<span class="theme_accent">', '</span>'), $logo_text).'</span>' : ''); ?><?php echo (esc_url($logo_image) ? '<img src="'.esc_url($logo_image).'" alt="">' : ''); ?></a></div>
					<?php 
					} 
				}
