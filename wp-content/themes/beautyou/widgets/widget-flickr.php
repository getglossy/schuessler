...
class themerex_flickr_widget extends WP_Widget {
	...
	function themerex_flickr_widget() {
		...
		/* Create the widget. */
		$this->WP_Widget( 'themerex-flickr-widget', __('AxiomThemes - Flickr photos', 'themerex'), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		<div class="flickr_images">
			...
			if ($flickr_count <= 10) {
				...
				?>
				<script type="text/javascript" src="http://www.flickr.com/badge_code_v2.gne?count=<?php echo esc_attr($flickr_count); ?>&amp;display=random&amp;flickr_display=random&amp;size=<?php echo esc_attr($size); ?>&amp;layout=x&amp;source=user&amp;user=<?php echo esc_attr($flickr_username); ?>"></script>
				<?php
			} else {
				// New method > 10 images
				$size = $THEMEREX_CURRENT_SIDEBAR == 'top' ? 'mid' : 'square';
				?>
				<script type="text/javascript" src="http://www.flickr.com/badge_code.gne?count=<?php echo esc_attr($flickr_count); ?>&amp;display=random&amp;flickr_display=random&amp;size=<?php echo esc_attr($size); ?>&amp;layout=x&amp;source=user&amp;nsid=<?php echo esc_attr($flickr_username); ?>&amp;raw=1"></script>
				<?php
			}
			?>
		</div>
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'flickr_username' )); ?>"><?php _e('Flickr ID (<a href="'.esc_url('http://www.idgettr.com').'" target="_blank">idGettr</a>):', 'themerex'); ?></label>
