...
class themerex_advert_widget extends WP_Widget {
	...
	function themerex_advert_widget() {
		...
		/* Create the widget. */
		$this->WP_Widget( 'themerex-advert-widget', __('AxiomThemes - Advertisement block', 'themerex'), $widget_ops, $control_ops );
	}
...
?>			
		<div class="widget_advert_inner">
			<?php
			if ($advert_image!='') {
				echo ($advert_link!='' ? '<a href="' . esc_url($advert_link) . '" class="image_wrapper">' : '') . '<img src="' . esc_url($advert_image) . '" alt="' . esc_html($title) . '" />' . ($advert_link!='' ? '</a>': '');
			}
			if ($advert_code!='') {
