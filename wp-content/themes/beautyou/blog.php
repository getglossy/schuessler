...
?>
	<div class="masonryWrap">
		...
		<?php } ?>
		<section class="masonry <?php echo 'masonry-colums-'.themerex_substr($blog_style, -1); ?> <?php echo get_custom_option('show_filters')=='yes' ? 'isotope' :  'isotopeNOanim'; ?>" data-columns="<?php echo themerex_substr($blog_style, -1); ?>">
<?php
} else if (themerex_strpos($blog_style, 'portfolio')!==false) {
?>
	<div class="portfolioWrap<?php echo get_custom_option('show_portfolio_spacing')=='no' ? ' NOspacing' :  ''; ?>">
		<?php if (get_custom_option('show_filters')=='yes') { ?>
		<div class="isotopeFiltr"></div>
		<?php } ?>
		<section class="masonry <?php echo get_custom_option('show_portfolio_spacing')=='no' ? 'portfolioNOspacing' :  ''; ?> <?php echo 'masonry-colums-'.themerex_substr($blog_style, -1); ?> <?php echo get_custom_option('show_filters')=='yes' ? 'isotope' :  'isotopeNOanim'; ?>" data-columns="<?php echo themerex_substr($blog_style, -1); ?>">
<?php
} else if (themerex_strpos($blog_style, 'grid')!==false) {
?>
	<div class="portfolioWrap NOspacing">
	<?php if (get_custom_option('show_filters')=='yes') { ?>
		<div class="isotopeFiltr"></div>
	<?php } ?>
	<section class="masonry grid portfolioNOspacing <?php echo get_custom_option('show_filters')=='yes' ? 'isotope' :  'isotopeNOanim'; ?>">
<?php
}
...
while ( have_posts() ) { the_post();
	...
	$post_args = array(
		'layout' => in_array(themerex_substr($blog_style, 0, 7), array('classic', 'masonry', 'portfol')) ? themerex_substr($blog_style, 0, 7) : themerex_substr($blog_style, 0, 7),
		'number' => $post_number,
		'add_view_more' => false,
	...
	if (in_array($pagination, array('viewmore', 'infinite'))) {
		if ($page_number < $wp_query->max_num_pages) {
			...
			<div id="viewmore" class="squareButton pagination_<?php echo esc_attr($pagination); ?>">
				<a href="#" id="viewmore_link" class="theme_button view_more_button"><span class="icon-spin3 viewmore_loading"></span><span class="viewmore_text_1"><?php _e('Load more', 'themerex'); ?></span><span class="viewmore_text_2"><?php _e('Loading ...', 'themerex'); ?></span></a>
				<script type="text/javascript">
					...
					'blog_style' => esc_attr($blog_style),
					'show_sidebar_main' => esc_attr($show_sidebar_main),
					'parent_cat_id' => esc_attr($parent_cat_id),
					'filters' => get_custom_option('show_filters')=='yes' ? get_custom_option('filter_taxonomy') : '',
					'hover' => '',
					'hover_dir' => '',
					'ppp' => esc_attr($ppp)
