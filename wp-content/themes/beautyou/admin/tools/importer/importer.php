...
class TRX_Importer {
	...
	function build_page() {
		...
		<div class="trx_importer">
			...
			<?php if (empty($this->success) && $this->options['enable_exporter']) { ?>
				<p>&nbsp;</p>
				<h2 class="trx_title"><?php _e('AxiomThemes Exporter', 'themerex'); ?></h2>
	...
	function checkRequiredPlugins() {
		...
		if (is_plugin_inactive('the-events-calendar/the-events-calendar.php'))
			$not_installed .= '<br>The Events Calendar plugin';
		if ($not_installed) {
