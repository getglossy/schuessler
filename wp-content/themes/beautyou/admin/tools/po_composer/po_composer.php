...
class PO_Composer {
	...
	function build_page() {
		...
		<div class="po_composer">
			<h2 class="po_composer_title"><?php _e('AxiomThemes .PO-files Composer', 'themerex'); ?></h2>
			...
			<form id="po_composer_form" action="#" method="post" enctype="multipart/form-data">
				...
				<div class="po_composer_buttons">
					<a href="#" id="po_save"><?php echo (esc_url($this->po_src)=='' ? __('Upload', 'themerex') : __('Update', 'themerex')); ?></a>
			...
			if (empty($this->error)) {
				...
				if (empty($this->po_src2) && !empty($_POST['po_text']) && substr($this->po_src, 0, 7)=='parent_' || substr($this->po_src, 0, 6)=='child_') {
					...
					if (is_writeable($po_file)) {
						...
						$this->error_update = sprintf(__('Error writing in language file "<b>%s.po</b>"!<br>Please, copy text from textarea below and paste it in the .po-file manually<br>or use links to upload .po and .mo files below the text area!', 'themerex'), $name);
					}
				}
			} else {
				$this->po_text = stripslashes($_POST['po_text']);
