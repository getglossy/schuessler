...
if (!function_exists('showPagination')) {
	function showPagination($args=array()) {
		$args = array_merge(array(
			...
			'prev_text' => __("&laquo;", 'themerex'),
			'next_text' => __("&raquo;", 'themerex'),
			'dot_text' => "&hellip;",
			'before' => '',
			'after' => '',
			),  is_array($args) ? $args
			...
			if ($max_page > 1) {
				...
				if ($opt['style'] == 'pages') {
					...
					if ($opt['pages_text']) {
						...
						$output .= '<li class="pager_pages '.$opt['button_class'].'"><span>' . esc_html($pages_text) . '</span></li>';
					}
					if ($cur_page > 1) {
						// First page
					...
					for ($i = 1; $i <= $max_page; $i++) {
						...
						if ($group == $cur_group) {
							if ($i < $show_pages_start) {
								if (!$dot1) {
									$output .= '<li class="pager_dot '.$opt['button_class'].'"><a href="'.esc_url(get_pagenum_link($show_pages_start-1)).'">'.esc_html($opt['dot_text']).'</a></li>';
									$dot1 = true;
								}
							} else if ($i > $show_pages_end) {
								if (!$dot2) {
									$output .= '<li class="pager_dot '.$opt['button_class'].'"><a href="'.esc_url(get_pagenum_link($show_pages_end+1)).'">'.esc_html($opt['dot_text']).'</a></li>';
									$dot2 = true;
								}
							} else if ($i == $cur_page) {
								...
								$output .= '<li class="'.$opt['button_class'].'"><a href="'.esc_url(get_pagenum_link($i)).'">'.esc_html($page_text).'</a></li>';
							}
						}
					}
					...
					if ($cur_page > 1) {
						...
						$output .= '<li class="pager_first ico left '.$opt['button_class'].'"><a href="'.esc_url(get_pagenum_link()).'">'.esc_html($page_text).'</a></li>';
						// Prev page
						$page_text = str_replace("&laquo;", '', $opt['prev_text']);
						$output .= '<li class="pager_prev ico left '.$opt['button_class'].'"><a href="'.esc_url(get_pagenum_link($cur_page-1)).'">'.esc_html($page_text).'</a></li>';
					}
					// Page XX from XXX
					if (empty($opt['pages_text']))
						$opt['pages_text'] = __('Page %CURRENT_PAGE% of %TOTAL_PAGES%', 'themerex');
					$pages_text = str_replace("%CURRENT_PAGE%", '<input class="navInput" readonly="readonly" type="text" size="1" value="'.esc_attr($cur_page).'">', $opt['pages_text']);
					$pages_text = str_replace("%TOTAL_PAGES%", number_format_i18n($max_page), $pages_text);
					...
					for ($i = 1; $i <= $max_page; $i++) {
						...
						if ($i == $cur_page) {
							$page_text = str_replace("%PAGE_NUMBER%", number_format_i18n($i), $opt['current_text']);
							$output .= '<td><a href="#" class="active">'.esc_html($page_text).'</a></li>';
						} else {
							$page_text = str_replace("%PAGE_NUMBER%", number_format_i18n($i), $opt['page_text']);
							$output .= '<td><a href="'.esc_url(get_pagenum_link($i)).'">'.esc_html($page_text).'</a></td>';
						}
					}
...
if (!function_exists('getThemeRexCalendar')) {
	function getThemeRexCalendar($onlyFirstLetter = false, $get_month = 0, $get_year = 0, $opt=array()) {
		...
		if ( !$posts ) {
			$gotsome = $wpdb->get_var($wpdb->prepare(
				"SELECT 1 as test FROM $wpdb->posts WHERE post_type = %s AND post_status = %s  LIMIT 1",$post_type_prepare,$post_status_prepare
			));
			if ( !$gotsome ) {
				$cache[ $key ] = '';
		...
		if ( $prev_year+$prev_month > 0 ) {
			$calendar_output .= '<div class="left"><a href="#" data-type="'.esc_attr($post_type_full).'" data-year="' . $prev_year . '" data-month="' . $prev_month . '" title="' . esc_attr( sprintf(__('View posts for %1$s %2$s', 'themerex'), $wp_locale->get_month($prev_month), date('Y', mktime(0, 0, 0, $prev_month, 1, $prev_year)))) . '">'
		...
		if ( $next_year+$next_month > 0 ) {
			$calendar_output .= '<div class="right"><a href="#" data-type="'.esc_attr($post_type_full).'" data-year="' . $next_year . '" data-month="' . $next_month . '" title="' . esc_attr( sprintf(__('View posts for %1$s %2$s', 'themerex'), $wp_locale->get_month($next_month), date('Y', mktime(0, 0 , 0, $next_month, 1, $next_year))) ) . '">'
...
if (!function_exists('getThumbSizes')) {
	function getThumbSizes($opt) {
		...
		$thumb_sizes = array(
			// 16:9
			'fullpost'          => array('w' => 1170,'h' => $opt['thumb_crop'] ? 647 : null, 'h_crop' => 647),	//1170 x 647
			'excerpt'           => array('w' => 760, 'h' => $opt['thumb_crop'] ? 420 : null, 'h_crop' => 420),	// 760 x 420
			'excerpt2'           => array('w' => 700, 'h' => $opt['thumb_crop'] ? 420 : null, 'h_crop' => 420),	// 700 x 420
			'related'           => array('w' => 714, 'h' => $opt['thumb_crop'] ? 510 : null, 'h_crop' => 510),	// 714 x 510
			'single-standard'   => array('w' => 1170,'h' => $opt['thumb_crop'] ? 647 : null, 'h_crop' => 647),	//1170 x 647
			'single-portfolio'  => array('w' => 1170,'h' => $opt['thumb_crop'] ? 647 : null, 'h_crop' => 647),	//1170 x 647
			'single-portfolio-fullscreen'
								=> array('w' => null,'h' => null,                            'h_crop' => null),	//Fullscreen mode - original image
			'image_tiny'        => array('w' => 250, 'h' => $opt['thumb_crop'] ? 200 : null, 'h_crop' => 200),	// 250 x  200
			'image_small'       => array('w' => 400, 'h' => $opt['thumb_crop'] ? 280 : null, 'h_crop' => 280),	// 400 x  280
			'image_medium'      => array('w' => 714, 'h' => $opt['thumb_crop'] ? 402 : null, 'h_crop' => 402),	// 714 x 402
			'image_large'       => array('w' => 1170, 'h' => $opt['thumb_crop'] ? 647 : null, 'h_crop' => 647),	// 1170 x 647
			'masonry2'          => array('w' => 714, 'h' => $opt['thumb_crop'] ? 402 : null, 'h_crop' => 402),	// 714 x 402
			'masonry3'          => array('w' => 400, 'h' => $opt['thumb_crop'] ? 225 : null, 'h_crop' => 225),	// 400 x 225
			'masonry4'          => array('w' => 350, 'h' => $opt['thumb_crop'] ? 280 : null, 'h_crop' => 280),	// 350 x 280
			'classic1'          => array('w' => 1170,'h' => $opt['thumb_crop'] ? 647 : null, 'h_crop' => 647),	//1170 x 647
			'classic2'          => array('w' => 714, 'h' => $opt['thumb_crop'] ? 402 : null, 'h_crop' => 402),	// 714 x 402
			'classic3'          => array('w' => 400, 'h' => $opt['thumb_crop'] ? 280 : null, 'h_crop' => 280),	// 400 x 280
			'classic4'          => array('w' => 350, 'h' => $opt['thumb_crop'] ? 280 : null, 'h_crop' => 280),	// 350 x 280
			'portfolio1'        => array('w' => 1170, 'h' => $opt['thumb_crop'] ? 647 : null, 'h_crop' => 647),	// 1170 x 647
			'portfolio2'        => array('w' => 1170, 'h' => $opt['thumb_crop'] ? 870 : null, 'h_crop' => 870),	// 1170 x 870
			// Non 16:9
			'events'   => array('w' => 714, 'h' => $opt['thumb_crop'] ? 600 : null, 'h_crop' => 600),	// 700 x 600
			'portfolio3'   => array('w' => 714, 'h' => $opt['thumb_crop'] ? 540 : null, 'h_crop' => 540),	// 714 x 540
			'portfolio4'   => array('w' => 550, 'h' => $opt['thumb_crop'] ? 400 : null, 'h_crop' => 400),	// 550 x 400
			'widgets'      => array('w' =>  75, 'h' => $opt['thumb_crop'] ?  75 : null, 'h_crop' =>  75),	//  75 x  75
			'shop'      => array('w' =>  200, 'h' => $opt['thumb_crop'] ?  200 : null, 'h_crop' =>  200)	//  200 x 200
		);
	...
	function getPostData(&$opt, $post_obj=null) {
		...
		if ($post_format == 'gallery') {
			...
			if ($post_audio!='' && $opt['substitute_audio']=='yes') {
				...
				if ($src) $post_audio = substituteAudio('<audio title="'.$tag_t.'" author="'.$tag_at.'" src="'.$src.'"></audio>');
			}
		}
	...
	function substituteAudio($post_text) {
		...
		while (($pos_start = themerex_strpos($post_text, themerex_substr($tag, 0, -1).' ', $pos_start+1))!==false) {
			...
			if ($src != '') {
				...
				$tag_t = getTagAttrib($tag_text, $tag, 'title');
				$tag_at = getTagAttrib($tag_text, $tag, 'author');
				$pos_end = $pos_end2!==false ? $pos_end2+8 : $pos_end+1;
				$container = '<div'.($id ? ' id="'.$id.'"' : '').' class="audio_container' . (($tag_t or $tag_at) ? ' with_info' : '') .''. ($tag_a ? ' align'.$tag_a : '') . '"' . ($tag_s || $tag_w || $tag_h ? ' style="'.($tag_w!='' ? 'width:' . $tag_w . (themerex_substr($tag_w, -1)!='%' ? 'px' : '') . ';' : '').($tag_h!='' ? 'min-height:' . $tag_h . 'px;' : '') . $tag_s . '"' : '') . '>
				'.($tag_at ? '<span class="audio_info">'.$tag_at.'</span>' : '').''.($tag_t ? '<h4 class="audio_info">'.$tag_t.'</h4>' : '').'';
	...
	function showShareSocialLinks($args) {
		...
		foreach ($args['share'] as $s => $data) {
			...
			$output .= (in_array($args['style'], array('list', 'drop')) ? '<li>' : '')
				. '<a href="' . ($args['popup'] ? '#' : esc_attr($link)) . '" class="'.$data['icon'].' social_icons share-item' . ($args['use_icons'] ? ' ' . $data['icon'] : '').'"'
...
if (!function_exists('showShareButtons')) {
	function showShareButtons($post_data) {
		...
		if ($rez && !empty($post_data['echo']) && $post_data['echo']) echo balanceTags($rez);
		return $rez;
	}
}
		...
		foreach ($args['allowed'] as $s) {
			if (array_key_exists($s, $THEMEREX_user_social_list)) {
				...
				if ($link) {
					...
					$output .= $args['before']
						. '<a href="' . $link . '" class="social_icons social_' . esc_attr($s) . ' ' . 'icon-'.esc_attr($s) . '" target="_blank"'
						. '>'
						. '</a>'
						. $args['after'];
				}
...
// Return image dimensions for Portfolio Grid
