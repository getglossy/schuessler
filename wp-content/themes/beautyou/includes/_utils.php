...
if (!function_exists('getStyleString')) {
	function getStyleString($top='',$right='',$bottom='',$left='',$width='',$height='') {
		...
		foreach ($top as $k=>$v) {
			// if $v == 0 -> continue
			if($v == 'mini' || $v == 'small' || $v == 'middle' || $v == 'big' || $v == 'large' || $v == '!mini' || $v == '!small' || $v == '!middle' || $v == '!big' || $v == '!large') { continue; }
			$imp = themerex_substr($v, 0, 1);
			if ($imp == '!') $v = themerex_substr($v, 1);
			if ($v != '' && $v != 'none') $output .= ($k=='width' ? 'width' : ($k=='height' ? 'height' : 'margin-'.$k)) . ':' . getStyleValue($v) . ($imp=='!' ? ' !important' : '') . ';';
		}
		return $output;
	}
}

// Return string for the style class
if (!function_exists('getStyleMargin')) {
	function getStyleMargin($top='',$right='',$bottom='',$left='') {
		if (!is_array($top)) {
			$top = compact('top', 'right', 'bottom', 'left');
		}
		$output = '';
		foreach ($top as $k=>$v) {
			if ($v != '' && is_string($v) && $v != '0' && (int)$v == 0 && $v != 'none') $output .= ' margin_'.$k .'_'.$v;
		}
		return $output;
	}
}
