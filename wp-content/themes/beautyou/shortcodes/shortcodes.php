...
function sc_accordion($atts, $content=null){	
	...
	global $THEMEREX_sc_accordion_counter, $THEMEREX_sc_accordion_large, $THEMEREX_sc_accordion_show_counter, $animation_all;
	$THEMEREX_sc_accordion_counter = 0;
	...
	return '<div' . ($id ? ' id="' . $id . '"' : '') 
			...
			. ((sc_param_is_on($animation_all) && ($animation && $animation != 'none')) ? ' data-animation="animated '.$animation.'"' : ''). '>'
...
function sc_audio($atts, $content = null) {
 ...
 . ' src="' . $src . '" class="sc_audio" ' . (sc_param_is_on($controls) ? ' controls="controls"' : '') . (sc_param_is_on($autoplay) && is_single() ? ' autoplay="autoplay"' : '') . ' width="' . $width . '" height="' . $height .'"'.($s!='' ? ' style="'.$s.'"' : '').''.$data.'></audio>';
}
// ---------------------------------- [/trx_audio] ---------------------------------------
...
function sc_banner($atts, $content=null){	
	...
	global $animation_all;
	return empty($src) ? '' : ('<a href="' . (empty($link) ? '#' : esc_url($link)) . '"' . ((sc_param_is_on($animation_all) && ($animation && $animation != 'none')) ? ' data-animation="animated '.$animation.'"' : '') .''. ($id ? ' id="' . $id . '"' : '')
...
function sc_blogger($atts, $content=null){	
	...
	global $THEMEREX_sc_blogger_busy, $THEMEREX_sc_blogger_counter, $animation_all, $post;
	...
	if (!in_array($style, array('regular','date','image_large','image_medium','image_small','image_tiny','accordion_1','accordion_2','list','excerpt','excerpt2','related'))
		&& !in_array(themerex_substr($style,0,7), array('classic','masonry','portfol')) && !in_array($style, array('grid')))
		$style='regular';
	if (!empty($ids)) {
	...
	$output = ($style=='list' ? '<ul' : '<div')
			...
			. ' class="sc_blogger'
				...
				. ($style=='related' ? ' relatedPostWrap no_indent_style columnsFloat' : '')
				. (sc_param_is_on($scroll) && sc_param_is_on($controls) ? ' sc_scroll_controls sc_scroll_controls_'.esc_attr($dir) : '')
				. ($descr == 0 ? ' no_description' : '')
				. '"'
			. ($s!='' ? ' style="'.$s.'"' : '')
		. '>'
		. ($dir!='vertical' && $style!='date' && !in_array(themerex_substr($style,0,7), array('classic','masonry','portfol', 'excerpt', 'grid')) ? '<div class="columnsWrap">' : '')
		. ($style!='date' && in_array(themerex_substr($style,0,7), array('grid')) ? '<div class="portfolioWrap NOspacing">' : '')
	...
	if (themerex_strpos($style, 'masonry')!==false || themerex_strpos($style, 'classic')!==false) {
		...
		if (!sc_param_is_off($filters))
			$output .= '<div class="isotopeFiltr"></div>';
		$output .= '<section class="masonry '.(!sc_param_is_off($filters) ? 'isotope' : 'isotopeNOanim').' masonry-colums-'.themerex_substr($style, -1).'" data-columns="'.themerex_substr($style, -1).'">';
	} else if (themerex_strpos($style, 'grid')!==false) {
		if (!sc_param_is_off($filters))
			$output .= '<div class="isotopeFiltr"></div>';
		$output .= '<section class="masonry grid portfolioNOspacing '.(!sc_param_is_off($filters) ? 'isotope' : 'isotopeNOanim').'">';
	}

	$args = array(
		'post_status' => current_user_can('read_private_pages') && current_user_can('read_private_posts') ? array('publish', 'private') : 'publish',
		'posts_per_page' => esc_attr((int)$count),
	...
	while ( $query->have_posts() ) { $query->the_post();
		...
		$args = array(
			'layout' => in_array(themerex_substr($style, 0, 7), array('classic', 'masonry', 'portfol', 'excerpt', 'related', 'grid')) || in_array($style, array('excerpt2')) ? themerex_substr($style, 0, 7) : 'blogger',
			'show' => false,
			...
			"tags_list" => in_array(themerex_substr($style, 0, 7), array('classic', 'masonry', 'portfol', 'related', 'grid')) || $filters=='tags',
	...
	if (in_array(themerex_substr($style, 0, 7), array('classic', 'masonry', 'portfol', 'grid'))) {
		if (themerex_strpos($style, 'masonry')!==false || themerex_strpos($style, 'classic')!==false) {
			$output .= '</section>';
		} else if (themerex_strpos($style, 'portfolio')!==false) {
			$output .= '</section>';
			...
			if ($filters == 'categories') {			// Use categories as filter items
				...
				if (count($flt_ids) > 0) {
					...
					foreach ($flt_ids as $flt_id=>$flt_name) {
						$filters_list .= '<li class="squareButton"><a href="#" data-filter=".flt_'.esc_attr($flt_id).'">'.$flt_name.'</a></li>';
					}
				}
			}
			if ($filters_list) {
	...
	$output	.= (sc_param_is_on($scroll) 
			? '</div><div id="'.esc_attr($id).'_scroll_bar" class="sc_scroll_bar sc_scroll_bar_'.esc_attr($dir).' '.esc_attr($id).'_scroll_bar"></div></div>'
...
/*
[trx_button id="unique_id" curve="0|1" type="square|round" fullsize="0|1" style="global|dark" size="medium|big|banner" icon="icon-name" link='#' target='']Button caption[/trx_button]
*/
function sc_button($atts, $content=null){	
	...
	return '<'.(sc_param_is_on($block) ? 'div' : 'span').''.((sc_param_is_on($animation_all) && ($animation && $animation != 'none')) ? ' data-animation="animated '.$animation.'"' : '').' class="sc_button sc_button_style_' . $style . ' sc_button_size_' . $size . ($align && $align!='none' ? ' align'.$align : '') . (!empty($class) ? ' '.$class : '')
        . ' ' . $type . 'Button' . (sc_param_is_on($fullsize) ? ' fullSize' : '') . ' ' . $style . ' ' . $size . ($icon!='' ? '  ico' : '') . '"'
		. ($s!='' ? ' style="'.$s.'"' : '') . '>'
		. '<a href="' . (empty($link) ? '#' : esc_url($link)) . '"' . (!empty($target) ? ' target="' . $target . '"' : '') . (!empty($rel) ? ' rel="' . $rel . '"' : '')
		. ($id ? ' id="' . $id . '"' : '') 
...
function sc_chat($atts, $content=null){	
	...
	return '<div' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_chat' . (!empty($class) ? ' '.$class : '') . '"' .((sc_param_is_on($animation_all) && ($animation && $animation != 'none')) ? ' data-animation="animated '.$animation.'"' : '') . ($s ? ' style="'.$s.'"' : '') . '>'
		. ($title == '' ? '' : ('<p class="sc_quote_title">' . ($link!='' ? '<a href="'.esc_url($link).'">' : '') . $title . ($link!='' ? '</a>' : '') . '</p>'))
		. $content
		.'</div>';
}
...
function sc_columns($atts, $content=null){	
	...
	return '<div' . ($id ? ' id="' . $id . '"' : '') . ' class="'. (sc_param_is_off($indentation) ? 'columnsFloat ' : '') .''. (sc_param_is_on($autoheight) ? 'autoHeight ' : '') .'columnsWrap sc_columns sc_columns_count_' . $count . (!empty($class) ? ' '.$class : '') . '"'. ((sc_param_is_on($animation_all) && ($animation && $animation != 'none')) ? ' data-animation="animated '.$animation.'"' : '') .($s!='' ? ' style="'.$s.'"' : '').'>' . do_shortcode($content).'</div>';
}
...
function sc_column_item($atts, $content=null) {
	...
	$class .= sc_param_is_on($bg_resize) ? '' :' resize_off';
...
//[trx_contact_form id="unique_id" style="main|order" title="Contact Form" description="Mauris aliquam habitasse magna a arcu eu mus sociis? Enim nunc? Integer facilisis, et eu dictumst, adipiscing tempor ultricies, lundium urna lacus quis."]
...
function sc_contact_form($atts, $content = null) {
	...
	if($style!='order') {
		...
		return '<div ' . ($id ? ' id="' . $id . '"' : '') . 'class="'.($style_color=='dark' ? 'dark' : 'light').' sc_contact_form sc_contact_form_' . ($content ? 'custom' : 'contact') . (!empty($class) ? ' ' . $class : '') . '"' .((sc_param_is_on($animation_all) && ($animation && $animation != 'none')) ? ' data-animation="animated '.$animation.'"' : '') . ($s != '' ? ' style="' . $s . '"' : '') . '>'
		...
		. ($content != ''
			...
			. '<input placeholder="' . __('Name:', 'themerex') . '" id="sc_contact_form_username" type="text" name="username">'
			. '</div>'
			. '<div class="columns1_2">'
			. '<input placeholder="' . __('Email:', 'themerex') . '" id="sc_contact_form_email" type="text" name="email">'
			. '</div>'
			. '</div>'
			. '<div class="message">'
			. '<textarea placeholder="' . __('Message', 'themerex') . '" id="sc_contact_form_message" class="textAreaSize" name="message"></textarea>'
			. '</div>'
			. '<div class="sc_contact_form_button">'
			. '<div class="squareButton comm sc_button_size_big global big"><a href="#" class="sc_contact_form_submit">' . __('Send the message', 'themerex') . '</a></div>'
			. '</div>'
		)
		...
		themerex_enqueue_script( 'form', themerex_get_file_url('/js/form/modernizr.custom.js'), array('jquery'), null, false );
		themerex_enqueue_script( 'classie', themerex_get_file_url('/js/form/classie.js'), array('jquery'), null, false );
		themerex_enqueue_script( 'stepsForm', themerex_get_file_url('/js/form/stepsForm.js'), array('jquery'), null, false );

		return  '<div class="'.($style_color=='dark' ? 'dark' : 'light').' sc_contact_form sc_contact_form_order' . (!empty($class) ? ' ' . $class : '') . '"' .((sc_param_is_on($animation_all) && ($animation && $animation != 'none')) ? ' data-animation="animated '.$animation.'"' : '') . ($s != '' ? ' style="' . $s . '"' : '') . '>'
		. ($title ? '<h1 class="title">' . $title . '</h1>' : '')
		. ($description ? '<div class="description">' . $description . '</div>' : '')
			...
			.'<div class="simform-inner">'
				.'<ol class="questions">'
							.'<li class="dot">'
								...
								.'<input placeholder="' . __('Your name', 'themerex') . '" id="q1" name="q1" type="text"/>'
							.'</li>'
							.'<li class="dot">'
								.'<span><label for="q2">Your email address?</label></span>'
								.'<input placeholder="' . __('Email', 'themerex') . '" id="q2" name="q2" type="text"/>'
							.'</li>'
							.'<li class="dot">'
								.'<span><label for="q3">What kind of procedure you want to order?</label></span>'
								.'<input placeholder="' . __('Type of procedure', 'themerex') . '" id="q3" name="q3" type="text"/>'
							.'</li>'
							.'<li class="dot">'
								.'<span><label for="q4">What do you like/dislike about your hair now?</label></span>'
								.'<input placeholder="' . __('For example, do you like the length?', 'themerex') . '" id="q4" name="q4" type="text"/>'
							.'</li>'
							.'<li class="dot">'
								.'<span><label for="q5">Are you looking to try something new?</label></span>'
								.'<input placeholder="' . __('For example, would you like to add a fringe?', 'themerex') . '" id="q5" name="q5" type="text"/>'
							.'</li>'
							.'<li class="dot">'
								.'<span><label for="q6">Talk about your maintenance schedule.</label></span>'
								.'<input placeholder="' . __('How often will you return to the salon for cut and color?', 'themerex') . '" id="q6" name="q6" type="text"/>'
							.'</li>'
				.'</ol><!-- /questions -->'
		...
		.'<div class="continue_button sc_button sc_button_style_border_1 sc_button_size_big squareButton border_1 big">'
			.'<a href="#">'. __('next question', 'themerex').'</a>'
		.'</div>'
		. '</form>
...
function sc_content($atts, $content=null){	
	...
	$s .= (sc_param_is_on($uppercase) ? 'text-transform:uppercase;' : '');
	$s .= ($size != '' ? 'font-size:' . getStyleValue($size) . ';' : '');
	$s .= ($line_height != '' ? 'line-height:' . getStyleValue($line_height) . ';' : '');
	$s .= ($color !== '' ? 'color:' . $color . ';' : '');
	$class .= getStyleMargin($top, $right, $bottom, $left);
	$class .= ($align=='left' ? ' sc_alignleft' : ($align=='right' ? ' sc_alignright' : ($align=='center' ? ' sc_aligncenter' : '')));
	$output = '<div' . ($id ? ' id="' . $id . '"' : '') 
		. ' class="sc_content' .(sc_param_is_on($main) ? '' : ' main').''. ($class ? ' ' . $class : '') . '' .(sc_param_is_on($text_styling) ? ' text_styling' : ''). '"'
		. ((sc_param_is_on($animation_all) && ($animation && $animation != 'none')) ? ' data-animation="animated '.$animation.'"' : '')
		. ($s!='' ? ' style="'.$s.'"' : '').'>'
		. do_shortcode($content) 
		. '</div>';
...
function sc_countdown($atts, $content = null) {
	...
	if (date('Y-m-d H:i:s') < $date.' '.($time ? $time : '00:00:00')) {
		...
		$s = getStyleString($top, $right, $bottom, $left, $width, $height);
		$class .= getStyleMargin($top, $right, $bottom, $left);
		return '<div' . ($id ? ' id="' . $id . '"' : '').' class="sc_countdown_wrapper' . ($align && $align!='none' ? ' align' . $align : '') . (!empty($class) ? ' '.$class : '') .'"'. ((sc_param_is_on($animation_all) && ($animation && $animation != 'none')) ? ' data-animation="animated '.$animation.'"' : '') .($s ? ' style="'.$s.'"' : '').'><div class="sc_countdown" data-date="'.$date.'" data-time="'.$time.'"></div></div>';
	} else 
		return '';
}
// ---------------------------------- [/trx_countdown] ---------------------------------------
...
function sc_emailer($atts, $content = null) {
	...
	return '<div' . ($id ? ' id="' . $id . '"' : '').' class="sc_emailer inputSubmitAnimation' . ($align && $align!='none' ? ' sc_align' . $align : '') . (sc_param_is_on($open) ? ' sFocus opened' : ' radCircle') . (!empty($class) ? ' '.$class : '') . '"' . ((sc_param_is_on($animation_all) && ($animation && $animation != 'none')) ? ' data-animation="animated '.$animation.'"' : '') . ($s ? ' style="'.$s.'"' : '') . '>'
...
function sc_google_map($atts, $content = null) {
	...
	return '<div id="' . $id . '" class="sc_googlemap'. (!empty($class) ? ' '.$class : '').'"'.((sc_param_is_on($animation_all) && ($animation && $animation != 'none')) ? ' data-animation="animated '.$animation.'"' : '').($s!='' ? ' style="'.$s.'"' : '')
...
function sc_icon($atts, $content=null){	
	...
	$s2 = ($weight != '' ? 'font-weight:'. $weight.';' : '')
		. ((int) $size > 0 && $size <= 11 ? 'font-size:'.$size*1.4 .'px;' : '')
		. ((int) $size > 0 && $size > 11 ? 'font-size:'.$size .'px;' : '')
		. ($color != '' ? 'color:'.$color.';' : '')
		. ($bg_color != '' ? 'background-color:'.$bg_color.';' : '')
		. (($background == 'round' || $background == 'square') && (int) $size > 0 ? ($s ? '' : 'display:inline-block;') . 'width:' . round($size*3) . 'px;height:' . round($size*3) . 'px;line-height:' . round($size*3) . 'px;' : '')
	;
	return $icon!='' 
		? '<'.(sc_param_is_on($link) ? 'a href="'.($url!='' ? esc_url($url) : '#').'"' : 'span') . '' . ($id ? ' id="' . $id . '"' : '')
			.' class="sc_icon '.$icon
			.($background && $background!='none' ? ' sc_icon_'.$background : '')
			.($bg_color != '' ? ' bg_icon' : ' no_bg_icon')
			.($align ? ' sc_align'.$align : '')
			.(!empty($class) ? ' '.$class : '')
			.'"'
			.((sc_param_is_on($animation_all) && ($animation && $animation != 'none')) ? ' data-animation="animated '.$animation.'"' : '')
			.($s || $s2 ? ' style="'.($s ? 'display:block;' : '') . $s . $s2 . '"' : '')
			.'>'.(sc_param_is_on($link) ? '</a>' : '</span>')
		: '';
}
...
function sc_image($atts, $content=null){	
	...
	if (sc_param_is_on($link) && $url_color > 0) {
		$attach_color = wp_get_attachment_image_src($url_color, 'full');
		if (isset($attach_color[0]) && $attach_color[0]!='') {
			$url_color = $attach_color[0];
		}
	}
	if (!empty($width) || !empty($height)) {
		...
		if ($w || $h) {
			$src = getResizedImageURL($src, $w, $h);
			$url_color = getResizedImageURL($url_color, $w, $h);
		}
	}
	if(sc_param_is_on($link)){
		return empty($src) || empty($url_color) ? '' : ('<a href="'.esc_url(trim($href)).'"' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_image sc_image_hover' . ($align && $align != 'none' ? ' sc_image_align_' . $align : '') . (!empty($shape) ? ' sc_image_shape_' . $shape : '') . (!empty($class) ? ' ' . $class : '') . '"' . ((sc_param_is_on($animation_all) && ($animation && $animation != 'none')) ? ' data-animation="animated '.$animation.'"' : '') . ($s != '' ? ' style="' . $s . '"' : '') . '>'
			. '<img src="' . $src . '" alt="" />'
			.'<span class="hoverShadow"></span>'
			.'<span class="hoverLink"></span>'
			. '<img class="img_hover" src="' . esc_url($url_color) . '" alt="" />' . (trim($title) || trim($icon) ? '<figcaption><span' . ($icon ? ' class="icon ' . $icon . '"' : '') . '>' . $title . '</span></figcaption>' : '')
			. '</a>');
	}
		...
		return empty($src) ? '' : ('<figure data-image="' . $hoverSrc . '" data-title="' . trim($title) . '"' . ($id ? ' id="' . $id . '"' : '') . ' class="thumb hoverIncrease sc_image ' . ($align && $align != 'none' ? ' sc_image_align_' . $align : '') . (!empty($shape) ? ' sc_image_shape_' . $shape : '') . (!empty($class) ? ' ' . $class : '') . '"' . ((sc_param_is_on($animation_all) && ($animation && $animation != 'none')) ? ' data-animation="animated '.$animation.'"' : '') . ($s != '' ? ' style="' . $s . '"' : '') . '>'
			. '<img src="' . $src . '" alt="" />' . ((trim($title) || trim($icon)) && $shape == 'square' ? '<figcaption><span' . ($icon ? ' class="icon ' . $icon . '"' : '') . '>' . $title . '</span></figcaption>' : '')
			. '</figure>');
	}
	else {
		return empty($src) ? '' : ('<figure' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_image ' . ($align && $align != 'none' ? ' sc_image_align_' . $align : '') . (!empty($shape) ? ' sc_image_shape_' . $shape : '') . (!empty($class) ? ' ' . $class : '') . '"' . ((sc_param_is_on($animation_all) && ($animation && $animation != 'none')) ? ' data-animation="animated '.$animation.'"' : '') . ($s != '' ? ' style="' . $s . '"' : '') . '>'
...
/*
[trx_infobox id="unique_id" style="regular|info|success|error|warning" static="0|1"]Et adipiscing integer, scelerisque pid, augue mus vel tincidunt porta[/trx_infobox]
*/
function sc_infobox($atts, $content=null){	
	...
	$s = getStyleString($top, $right, $bottom, $left);
	$class .= getStyleMargin($top, $right, $bottom, $left);
	return '<div' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_infobox sc_infobox_style_' . $style . (sc_param_is_on($closeable) ? ' sc_infobox_closeable' : '') . (!empty($class) ? ' '.$class : '') . '"' . ((sc_param_is_on($animation_all) && ($animation && $animation != 'none')) ? ' data-animation="animated '.$animation.'"' : '') .($s!='' ? ' style="'.$s.'"' : '').'>'
			. do_shortcode($content) 
			. '</div>';
}
...
/*
[trx_line id="unique_id" style="solid|dashed|dotted|double" top="margin_in_pixels" bottom="margin_in_pixels" width="width_in_pixels_or_percent" height="line_thickness_in_pixels" color="line_color's_name_or_#rrggbb"]
*/
function sc_line($atts, $content=null){	
	...
	return '<div' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_line' . ($style != '' ? ' sc_line_style_' . $style : '') . (!empty($class) ? ' '.$class : '') . '"' . ((sc_param_is_on($animation_all) && ($animation && $animation != 'none')) ? ' data-animation="animated '.$animation.'"' : '') .($s!='' ? ' style="'.$s.'"' : '').'></div>';
}
...
function sc_list($atts, $content=null){	
	...
	if ($style == 'arrows' && trim($icon) == '') $icon = 'icon-right-open-mini';
	$THEMEREX_sc_list_counter = 0;
	$THEMEREX_sc_list_icon = $icon;
	$THEMEREX_sc_list_style = $style;
	return '<' . ($style=='ol' ? 'ol' : 'ul') . ($id ? ' id="' . $id . '"' : '') . ' class="sc_list sc_list_style_' . $style . (!empty($class) ? ' '.$class : '') . '"' . ((sc_param_is_on($animation_all) && ($animation && $animation != 'none')) ? ' data-animation="animated '.$animation.'"' : '') . ($s!='' ? ' style="'.$s.'"' : '') . '>'
			. do_shortcode($content) 
			. '</' .($style=='ol' ? 'ol' : 'ul') . '>';
}
...
function sc_price($atts, $content=null){	
	...
	if (!empty($money)) {
		...
		if (count($m)==1) $m[1] = '';
		$output = '
			<div' .($s!='' ? ' style="'.$s.'"' : '').''. ($id ? ' id="' . $id . '"' : '').' class="sc_price_item'. (!empty($class) ? ' '.$class : '').'"'.((sc_param_is_on($animation_all) && ($animation && $animation != 'none')) ? ' data-animation="animated '.$animation.'"' : '').'>
...
/*
[trx_price_table id="unique_id" align="left|right|center" style="light|dark"]
...
function sc_price_table($atts, $content=null){	
	...
	$THEMEREX_sc_price_table_counter = 0;
	$count = max(1, $count);
	return '<div' . ($id ? ' id="' . $id . '"' : '') . ' class="'.($style ? 'sc_pricing_'.$style.' ' : '').'sc_pricing_table columns_' . $count . ($align && $align!='none' ? ' align'.themerex_strtoproper($align) : '') . (!empty($class) ? ' '.$class : '') . '"' . ((sc_param_is_on($animation_all) && ($animation && $animation != 'none')) ? ' data-animation="animated '.$animation.'"' : '') . ($s!='' ? ' style="'.$s.'"' : '') . '>'
			. do_shortcode($content)
		. '</div>';
}
	...
	if ($type=='price' && $money!='') {
		$content = '
			<div class="sc_price_item'. (!empty($class) ? ' '.$class : '').'">
				<span class="sc_price_currency">'.$currency.'</span>
				<div class="sc_price_money">'.$money.'</div>
				<div class="sc_price_info">
					<div class="sc_price_period">'.($period ? '/'.$period : '').'</div>
				</div>
			</div>
		';
...
function sc_quote($atts, $content=null){	
	...
	global $animation_all;
	$class .= getStyleMargin($top, '', $bottom, '');
	$cite_param = $cite != '' ? ' cite="' . $cite . '"' : '';
	$title = $title=='' ? $cite : $title;
	$content = do_shortcode($content);
	if (themerex_substr($content, 0, 2)!='<p') $content = '<p>' . $content . '</p>';
	return '<blockquote' . ($id ? ' id="' . $id . '"' : '') . $cite_param . ' class="sc_quote'. (!empty($class) ? ' '.$class : '').' sc_quote_style_' . $style.'"' . ((sc_param_is_on($animation_all) && ($animation && $animation != 'none')) ? ' data-animation="animated '.$animation.'"' : '') . ($s!='' ? ' style="'.$s.'"' : '') . '>'
		. $content
...
/*
[trx_section id="unique_id" class="class_name" style="css-styles" box="yes|no" dedicated="yes|no"]Et adipiscing integer, scelerisque pid, augue mus vel tincidunt porta[/trx_section]
*/
...
function sc_section($atts, $content=null){	
	...
	$class .= getStyleMargin($top, $right, $bottom, $left);
	$class .= (sc_param_is_on($border_style) ? ' styleBorder' : '');
	$s = getStyleString('!'.$top, '!'.$right, '!'.$bottom, '!'.$left, $width, $height)
		.($color !== '' ? 'color:' . $color . ';' : '')
		.($bg_color !== '' ? 'background-color:' . $bg_color . ';' : '')
		.($bg_image !== '' ? 'background-image:url(' . $bg_image . ');' : '')
...
function sc_skills($atts, $content=null){	
	...
	global $animation_all;
	global $THEMEREX_sc_skills_counter, $THEMEREX_sc_skills_columns, $THEMEREX_sc_skills_height, $THEMEREX_sc_skills_max, $THEMEREX_sc_skills_dir, $THEMEREX_sc_skills_type, $THEMEREX_sc_skills_color, $THEMEREX_sc_skills_legend, $THEMEREX_sc_skills_data, $THEMEREX_sc_skills_style, $THEMEREX_sc_skills_style_bar;
	$THEMEREX_sc_skills_counter = 0;
	$THEMEREX_sc_skills_columns = 0;
	$THEMEREX_sc_skills_height = 0;
	...
	if ($style_bar) $THEMEREX_sc_skills_style_bar = $style_bar = max(1, min(2, $style_bar));
...
function sc_skills_item($atts, $content=null) {
	...
	global $THEMEREX_sc_skills_counter, $THEMEREX_sc_skills_columns, $THEMEREX_sc_skills_height, $THEMEREX_sc_skills_max, $THEMEREX_sc_skills_dir, $THEMEREX_sc_skills_type, $THEMEREX_sc_skills_color, $THEMEREX_sc_skills_legend, $THEMEREX_sc_skills_data, $THEMEREX_sc_skills_style, $THEMEREX_sc_skills_style_bar, $THEMEREX_sc_skills_title;
	...
	if (empty($color)) $color = apply_filters('theme_skin_get_theme_color_1', $color);
	$color = apply_filters('theme_skin_get_theme_color', $color);
	if ($style) $style = max(1, min(4, $style));
	if ($style_bar) $style_bar = max(1, min(2, $style_bar));
	if (empty($style)) $style = $THEMEREX_sc_skills_style;
	if (empty($style_bar)) $style_bar = $THEMEREX_sc_skills_style_bar;
	...
	if ($THEMEREX_sc_skills_type=='arc') {
		...
		$output .= ($THEMEREX_sc_skills_columns > 0 ? '<div class="sc_skills_column columns1_'.$THEMEREX_sc_skills_columns.'">' : '')
				. '<div' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_skills_item' . ($style ? ' sc_skills_style_'.$style : '') . ($THEMEREX_sc_skills_type=='bar' && $style_bar ? ' sc_skills_style_bar_'.$style_bar : '') . ($THEMEREX_sc_skills_counter % 2 == 1 ? ' odd' : ' even') . ($THEMEREX_sc_skills_counter == 1 ? ' first' : '') . '"'
					. ($THEMEREX_sc_skills_height !='' ? ' style="height: '.$THEMEREX_sc_skills_height.';"' : '')
				. '>';

		if (in_array($THEMEREX_sc_skills_type, array('bar')) && $THEMEREX_sc_skills_dir=='horizontal') {
			$output .=
			...
			.'</div>'
			.$title_block
			.'<div class="sc_skills_wrap"><div class="sc_skills_count'. (!empty($class) ? ' '.$class : '').'"' . ($THEMEREX_sc_skills_type=='bar' && $color && ($style_bar == 1 || $style_bar == 2) ? ' style="background-color:' . $color . ';"' : '') . '>'
			. '</div></div>';
		}
		else if (in_array($THEMEREX_sc_skills_type, array('counter','bar'))) {
			$output .= '<div class="sc_skills_count'. (!empty($class) ? ' '.$class : '').'"' . ($THEMEREX_sc_skills_type=='bar' && $color ? ' style="background-color:' . $color . ';"' : '') . '>'
						. '<div class="sc_skills_total"'
							. ' data-start="'.$start.'"'
							. ' data-stop="'.$stop.'"'
							. ' data-step="'.$step.'"'
...
function sc_slider($atts, $content=null){	
	...
	$output = ($border!='none' 
				? '<div class="sc_border sc_border_'.$border.($align!='' && $align!='none' ? ' sc_align'.$align : '').''. (!empty($class) ? ' '.$class : '').'"'.($ms.$hs ? ' style="'.$ms.$hs.'"' : '').'>'
				: '')
			. (in_array($pagination, array('full', 'over')) 
				? '<div class="sc_slider_pagination_area sc_slider_pagination_'.$pagination.''.(!empty($class) ? ' '.$class : '').'"'
					.($ms.$hs ? ' style="'.$ms.$hs.'"' : '') .'>' 
				: '')
			. '<div' . ($id ? ' id="' . $id . '"' : '') 
			. ' class="sc_slider'
			...
			while ( $query->have_posts() ) { 
				...
				$output .= '<li' 
					...
					. ' style="'
						...
						. '"'
					. '>' 
					. (sc_param_is_on($links) ? '<a href="'.esc_url($post_link).'" title="'.htmlspecialchars($post_title).'">' : '')
...
function sc_slider_item($atts, $content=null) {
	...
	return empty($src) ? '' : ('<li' . ($id ? ' id="' . $id . '"' : '').' class="' . $THEMEREX_sc_slider_engine.'-slide' . (!empty($class) ? ' '.$class : '') . '"'
		...
		.'">' 
		. (sc_param_is_on($THEMEREX_sc_slider_links) ? '<a href="'.($src ? esc_url($src) : esc_url($url)).'"></a>' : '')
		. '</li>');
}
// ---------------------------------- [/trx_slider] ---------------------------------------
...
function sc_table($atts, $content=null){	
	...
	return '<div' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_table sc_table_style_' . max(1, min(4, $style)) . ' sc_table_size_' . $size . (!empty($align) ? ' sc_table_align_'.$align : '') . (!empty($class) ? ' '.$class : '') . '"'.((sc_param_is_on($animation_all) && ($animation && $animation != 'none')) ? ' data-animation="animated '.$animation.'"' : '').($s!='' ? ' style="'.$s.'"' : '') .'>'
			. do_shortcode($content) 
			. '</div>';
}
...
function sc_tabs($atts, $content = null) {
	...
	$s = getStyleString($top, $right, $bottom, $left, $width);
	$class .= getStyleMargin($top, $right, $bottom, $left);
	$s .= ($bg_color!='' ? 'background-color:'.$bg_color.';' : '');
	...
	$tabs_output = '<div' . ($id ? ' id="' . $id . '"' : '') . ' class="'.(sc_param_is_on($padding) ? 'padding ' : '').'   sc_tabs sc_tabs_style_'.$style. (!empty($class) ? ' '.$class : '') . '"'.((sc_param_is_on($animation_all) && ($animation && $animation != 'none')) ? ' data-animation="animated '.$animation.'"' : '').($s!='' ? ' style="'.$s.'"' : '') .' data-active="' . ($initial-1) . '">'
	...
	$tabs_output .= $titles_output
		. '</ul>'
		. ($style == 2 ? '<div class="sc_tabs_wrap">' : '')
		. $content
		. ($style == 2 ? '</div>' : '')
		.'</div>';
	return $tabs_output;
...
function sc_team($atts, $content=null){	
	...
	$THEMEREX_sc_team_counter = 0;
	$content = do_shortcode($content);
	return '<div' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_team'.(!empty($class) ? ' '.$class : '').'"'.((sc_param_is_on($animation_all) && ($animation && $animation != 'none')) ? ' data-animation="animated '.$animation.'"' : '').($s!='' ? ' style="'.$s.'"' : '') .'>'
...
function sc_team_item($atts, $content=null) {
	...
	if (!empty($user) && $user!='none' && ($user_obj = get_user_by('login', $user)) != false) {
		...
		if (empty($socials))	$socials = showUserSocialLinks(array('author_id'=>$user_obj->ID, 'echo'=>false, 'style'=>'icons', 'before'=>'<li>', 'after' => '</li>'));
	} else {
		...
		for ($i=0; $i<count($allowed); $i++) {
			...
			if (!empty($s[1]) && array_key_exists($s[0], $THEMEREX_user_social_list)) {
				$socials .= '<li><a href="' . esc_url($s[1]) . '" class="social_icons social_' . $s[0] . ' ' . 'icon-'.$s[0] . '" target="_blank">'
						. '</a></li>';
			}
		}
	}
	...
	if (!empty($name) && !empty($position)) {
		return '<div class="columns1_'.$THEMEREX_sc_team_count . (!empty($class) ? ' '.$class : '').'">'
				. '<div' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_team_item sc_team_item_' . $THEMEREX_sc_team_counter . ($THEMEREX_sc_team_counter % 2 == 1 ? ' odd' : ' even') . ($THEMEREX_sc_team_counter == 1 ? ' first' : '') . '">'
					. '<div class="sc_team_item_avatar'.(empty($socials) ? ' no_socials' : '').'">'
						. $photo
						. (!empty($socials) ? '<ul class="sc_team_item_socials">' . $socials . '</ul>' : '')
					. '</div>'
					. (!empty($link) ? '<div class="team_item_link"><a class="'.$icon.'" href="'.$link.'"></a></div>' : '')
					. '<div class="sc_team_item_info">'
					. '<h4 class="sc_team_item_title">' . $name . '</h4>'
					. '<div class="sc_team_item_position">' . $position . '</div>'
					. (!empty($descr) ? '<div class="sc_team_item_description">' . $descr . '</div>' : '')
					. '</div>'
				. '</div>'
			. '</div>';
	}
...
function sc_testimonials($atts, $content=null){	
	...
	return
		'<div' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_testimonials sc_testimonials_style'.(sc_param_is_off($controls) ? '' : ' sc_testimonials_controls_'.$controls) . (!empty($class) ? ' '.$class : '') .'"' . ((sc_param_is_on($animation_all) && ($animation && $animation != 'none')) ? ' data-animation="animated '.$animation.'"' : '') . ($s!='' ? ' style="'.$s.'"' : '') . '>'
			.($title ? '<h2 class="sc_title sc_title_underline sc_testimonials_title">'.$title.'</h2>' : '')
...
function sc_testimonials_item($atts, $content=null){	
	...
	if (empty($photo)) {
		...
		$photo = getResizedImageTag($photo, 70, 70);
	}
...
add_shortcode('trx_text_slider', 'sc_text_slider');

$THEMEREX_sc_text_slider_count = 0;
$THEMEREX_sc_text_slider_width = 0;
$THEMEREX_sc_text_slider_height = 0;
function sc_text_slider($atts, $content=null){
	if (in_shortcode_blogger()) return '';
	extract(shortcode_atts(array(
		"id" => "",
		"class" => "",
	...
	$class .= getStyleMargin($top, $right, $bottom, $left);
	$d = getStyleString('', '', '', '', $width, $height);
	global $THEMEREX_sc_text_slider_count, $THEMEREX_sc_text_slider_width, $THEMEREX_sc_text_slider_height;
	$THEMEREX_text_slider_count = 0;
	$THEMEREX_text_slider_width = getStyleValue($width);
	$THEMEREX_text_slider_height = getStyleValue($height);
	$content = do_shortcode($content);
	...
	return
	'<div' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_text_slider' . (!empty($class) ? ' '.$class : '') .'"' . ((sc_param_is_on($animation_all) && ($animation && $animation != 'none')) ? ' data-animation="animated '.$animation.'"' : '') . ($s!='' ? ' style="'.$s.'"' : '') . '>'
	. ($THEMEREX_sc_text_slider_count>1 ? '<div class="'.(sc_param_is_off($autoheight) ? '' : 'sc_slider_autoheight ').'sc_slider_controls sc_slider_pagination sc_slider sc_slider_swiper swiper-slider-container"'.($d ? ' style="'.$d.'"' : '') . ((int) $interval > 0 ? ' data-interval="'.$interval.'"' : '') . '>' : '')
	. '<ul class="sc_text_slider_items'.($THEMEREX_sc_text_slider_count>1 ? ' slides swiper-wrapper' : '').'">'
	. $content
	. '</ul>
	<div class="flex-control-nav"></div>'
	. ($THEMEREX_sc_text_slider_count>1 ? '</div>' : '')
	. '</div>';
}


add_shortcode('trx_text_slider_item', 'sc_text_slider_item');

function sc_text_slider_item($atts, $content=null){
	if (in_shortcode_blogger()) return '';
	extract(shortcode_atts(array(
		"id" => "",
		"class" => ""
	...
	global $THEMEREX_sc_text_slider_count, $THEMEREX_sc_text_slider_width, $THEMEREX_sc_text_slider_height;
	$THEMEREX_sc_text_slider_count++;

	return '<li' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_text_slider_item slides swiper-slide'. (!empty($class) ? ' '.$class : '').'" style="'.(!empty($THEMEREX_sc_text_slider_width) ? 'width:' . $THEMEREX_sc_text_slider_width . ';' : '').(!empty($THEMEREX_sc_text_slider_height) ? 'height:' . $THEMEREX_sc_text_slider_height . ';' : '').'">'
	. '<div class="sc_text_slider_item_content">'
	. '<div class="sc_text_slider_item_text">'.do_shortcode($content).'</div>'
	. '</div>'
	. '</li>';
}
...
function sc_title($atts, $content=null){	
	...
	$pic = $style!='iconed' 
		...
		: '<'.($size!='small' ? 'div' : 'span').' class="sc_title_'.($icon ? 'icon' : 'image').' sc_title_'.$position.' sc_size_'.$size
			...
			((sc_param_is_on($animation_all) && ($animation && $animation != 'none')) ? ' data-animation="animated '.$animation.'"' : '')
			.'style="'
			.(!empty($background) && $background!='none' && $bg_color!='' ? 'background-color:'.$bg_color.';' : '').''
			.(!empty($icon_color) && $icon_color!='none' && $icon_color!='' ? 'color:'.$icon_color.';' : '')
			.'">'
	...
	return ($size!='small' ? $pic : '')
		. '<h' . $type . ($id ? ' id="' . $id . '"' : '')
		. ' class="'.(!empty($align) ? 'sc_title_align_'.$align.' ' : '').'sc_title sc_title_'.$style. (!empty($class) ? ' '.$class : '').'"'
		. ((sc_param_is_on($animation_all) && ($animation && $animation != 'none')) ? ' data-animation="animated '.$animation.'"' : '')
		. ($s!='' ? ' style="'.$s.'"' : '')
		. '>'
		. ($size=='small' ? $pic : '')
...
function sc_twitter($atts, $content=null){	
	...
	if (!empty($twitter_consumer_key) && !empty($twitter_consumer_secret) && !empty($twitter_token_key) && !empty($twitter_token_secret)) {
		...
		if ($data && isset($data[0]['text'])) {
			$output = '
				<div' . ($id ? ' id="' . $id . '"' : '').' class="sc_twitter twitBlock'.($class ? ' '.$class : '').'"'. ((sc_param_is_on($animation_all) && ($animation && $animation != 'none')) ? ' data-animation="animated '.$animation.'"' : '') . ($s!='' ? ' style="'.$s.'"' : '').'>
			...
			foreach ($data as $tweet) {
				...
				$output .= '<li class="sc_twitter_item swiper-slide' . ($cnt==$twitter_count-1 ? ' last' : '') . '">'
						...
						. (sc_param_is_on($show_user) ? '<a href="https://twitter.com/' . $twitter_username . '" class="twitAuthor" target="_blank">@' . htmlspecialchars($tweet['user']['screen_name']) . '</a> ' : '')
...
function sc_zoom($atts, $content=null){	
	...
	return empty($src) ? '' : ((!sc_param_is_off($border) ? '<div class="sc_border sc_border_'.$border.'">' : '')
				.'<div' . ($id ? ' id="' . $id . '"' : '') . ' class="sc_zoom'. (!empty($class) ? ' '.$class : '').'"'.((sc_param_is_on($animation_all) && ($animation && $animation != 'none')) ? ' data-animation="animated '.$animation.'"' : '').($s!='' ? ' style="'.$s.'"' : '').'>'
...
function sc_sidebar($atts, $content=null){
	...
	return '<div class="widget_area sidebarStyleLight sc_sidebar_selector'.(!empty($class) ? $class : '').'"'.((sc_param_is_on($animation_all) && ($animation && $animation != 'none')) ? ' data-animation="animated '.$animation.'"' : '').(!empty($s) ? ' style="'.$s.'"' : '').'>'.$sidebar_content.'</div>';
}
// ---------------------------------- [/trx_sidebar] ---------------------------------------
...
function sc_events($atts, $content=null){
	...
	$output = '<div'
		. ($id ? ' id="' . $id . '"' : '')
		. ' class="sc_events masonryWrap'
		. ($class!='' ? ' '.$class : ''). '"'
		. ((sc_param_is_on($animation_all) && ($animation && $animation != 'none')) ? ' data-animation="animated '.$animation.'"' : '')
		. ($s!='' ? ' style="'.$s.'"' : '')
		. '>';

	$output .= '<section class="masonry isotopeNOanim" data-columns="'.esc_attr((int)$col).'">';
